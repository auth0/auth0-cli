...
has_toc: false
---
# auth0 guardian factors push update-fcm

Partially update the Google FCM (legacy) push-notification configuration.

## Usage
```
auth0 guardian factors push update-fcm [flags]
```

## Examples

```
  auth0 guardian factors push update-fcm --server-key <server-key>
```
...
```
      ...
      --server-key string   Google FCM (legacy) server key.
```
