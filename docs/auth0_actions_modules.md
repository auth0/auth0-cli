...
has_children: true
---
# auth0 actions modules

Action modules are reusable code libraries that actions can import. Manage them here, then associate a module with an action using the `--module` flag on `auth0 actions create` or `auth0 actions update`.
