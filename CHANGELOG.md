...
- Stream results as NDJSON (one JSON object per line) instead of a JSON array when a streaming command such as `auth0 logs tail` runs in JSON or agent mode, so a reader can consume records incrementally without waiting for an array that never closes while tailing
