...
func TestEventStreamPickerOptions(t *testing.T) {
	tests := []struct {
		name         string
		eventStreams []*management.EventStream
		apiError     error
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			eventStreamAPI := mock.NewMockEventStreamAPI(ctrl)
			eventStreamAPI.EXPECT().
				List(gomock.Any()).
				Return(&management.EventStreamList{
					EventStreams: test.eventStreams}, test.apiError)

			cli := &cli{
				api: &auth0.API{EventStream: eventStreamAPI},
			}

			options, err := cli.eventStreamPickerOptions(context.Background())
