...
// A .csproj with only mobile TFMs (no Microsoft.Maui reference) is dotnet-mobile,
// not MAUI. This matches the report Bug 4 fix.
func TestDetectProject_DotnetMobile_AndroidIOS(t *testing.T) {
	dir := t.TempDir()
	writeTestFile(t, dir, "MyApp.csproj",
...
// A .csproj with Microsoft.Maui reference alongside mobile TFMs is MAUI.
func TestDetectProject_MAUI_WithMauiReference(t *testing.T) {
	dir := t.TempDir()
	writeTestFile(t, dir, "MyApp.csproj",
		`<Project Sdk="Microsoft.NET.Sdk"><PropertyGroup><TargetFrameworks>net8.0-android;net8.0-ios</TargetFrameworks></PropertyGroup><ItemGroup><PackageReference Include="Microsoft.Maui.Controls" /></ItemGroup></Project>`)
...
// Manage.py is generated by django-admin startproject and is the primary signal.
...
// (fullstack project with a Django backend and a Vite-powered frontend).
func TestDetectProject_Django_ManagePy_BeatsViteConfig(t *testing.T) {
	dir := t.TempDir()
	writeTestFile(t, dir, "manage.py", `#!/usr/bin/env python`)
	writeTestFile(t, dir, "vite.config.ts", "")
...
func TestDetectProject_Django_RequirementsTxt(t *testing.T) {
	dir := t.TempDir()
...
func TestDetectProject_Django_Pyproject(t *testing.T) {
	dir := t.TempDir()
...
func TestDetectProject_Flask_Pipfile(t *testing.T) {
	dir := t.TempDir()
	writeTestFile(t, dir, "Pipfile", "[packages]\nflask = \"*\"\n")
...
func TestDetectProject_Django_Pipfile(t *testing.T) {
	dir := t.TempDir()
	writeTestFile(t, dir, "Pipfile", "[packages]\ndjango = \"*\"\n")
...
// Django in Pipfile.lock (JSON format) is detected as django.
func TestDetectProject_Django_PipfileLock(t *testing.T) {
	dir := t.TempDir()
	writeTestFile(t, dir, "Pipfile.lock", `{"default":{"django":{"version":"==4.2"}}}`)
...
// Flask takes priority over Django when both appear in the same file (first match wins).
func TestDetectProject_FlaskTakesPriorityOverDjango(t *testing.T) {
	dir := t.TempDir()
	writeTestFile(t, dir, "requirements.txt", "flask==2.0\ndjango==4.0\n")
...
// Bug 17: port must still be shown when framework is ambiguous but all candidates share a port.
...
// not svelte (spa). Plain Svelte+Vite also has vite.config.ts and a svelte dep, so the kit
// dep is the only reliable distinguisher.
func TestDetectProject_SvelteKitDepBeatsViteAndSvelteDep(t *testing.T) {
	dir := t.TempDir()
	// Simulate a real SvelteKit project: has vite.config.ts, svelte dep, AND @sveltejs/kit.
	writeTestFile(t, dir, "vite.config.ts", "")
	writeTestFile(t, dir, "svelte.config.js", "")
	writeTestFile(t, dir, "package.json", `{"devDependencies":{"@sveltejs/kit":"^2","svelte":"^5","vite":"^7"}}`)
...
func TestDetectProject_ExpoViaAppJSON(t *testing.T) {
	dir := t.TempDir()
	writeTestFile(t, dir, "app.json", `{"expo":{"name":"my-app","slug":"my-app"}}`)
	writeTestFile(t, dir, "package.json", `{"dependencies":{"expo":"~54.0.0","react-native":"0.81.5"}}`)
...
// Ionic start --type=angular generates angular.json in the project root.
func TestDetectProject_IonicAngularBeatsAngularJSON(t *testing.T) {
	dir := t.TempDir()
	writeTestFile(t, dir, "angular.json", `{}`)
	writeTestFile(t, dir, "package.json", `{"dependencies":{"@ionic/angular":"^7","@angular/core":"^17"}}`)
...
// Bug 3: @ionic/react must win over vite.config.ts + react dep.
func TestDetectProject_IonicReactBeatsViteAndReact(t *testing.T) {
	dir := t.TempDir()
	writeTestFile(t, dir, "vite.config.ts", "")
	writeTestFile(t, dir, "package.json", `{"dependencies":{"@ionic/react":"^8","react":"^18"}}`)
...
// Bug 3: @ionic/vue must win over vite.config.ts + vue dep.
func TestDetectProject_IonicVueBeatsViteAndVue(t *testing.T) {
	dir := t.TempDir()
	writeTestFile(t, dir, "vite.config.ts", "")
	writeTestFile(t, dir, "package.json", `{"dependencies":{"@ionic/vue":"^8","vue":"^3"}}`)
...
// Bug 8: jakarta.platform:jakarta.jakartaee-api is the standard Jakarta EE 9+ BOM;
// it must be detected as java-ee, not vanilla-java.
func TestDetectProject_JavaEE_JakartaPlatform(t *testing.T) {
	dir := t.TempDir()
...
// Bug 10: dotnet new mvc on .NET 6+ does NOT generate a Microsoft.AspNetCore.Mvc
// PackageReference - it only sets Sdk="Microsoft.NET.Sdk.Web". Must still detect as aspnet-mvc.
func TestDetectProject_AspnetMVC_DotNet6NoPkgRef(t *testing.T) {
	dir := t.TempDir()
	writeTestFile(t, dir, "MyApp.csproj",
...
// Bug 5: Laravel 10+ ships with vite.config.js; composer.json must take priority.
func TestDetectProject_LaravelBeatsViteConfig(t *testing.T) {
	dir := t.TempDir()
	writeTestFile(t, dir, "vite.config.js", "")
	writeTestFile(t, dir, "composer.json", `{"name":"my/laravel-app","require":{"laravel/framework":"^10.0"}}`)
...
func TestDetectFromCsproj(t *testing.T) {
	tests := []struct {
		...
		{
			// Mobile TFM without Microsoft.Maui reference → dotnet-mobile (Bug 4 fix).
			// Uses regex net\d+\.\d+-(?:android|ios) to avoid false positives.
			name:      "dotnet_mobile_android_target",
			content:   `<TargetFrameworks>net8.0-android</TargetFrameworks>`,
			wantFw:    "dotnet-mobile",
			wantType:  "native",
			wantFound: true,
		},
		{
			// Mobile TFM without Microsoft.Maui reference → dotnet-mobile (Bug 4 fix).
			name:      "dotnet_mobile_ios_target",
		...
		{
			// Bare "-ios" in a package name must NOT trigger dotnet-mobile (false-positive guard).
			name:      "bare_ios_in_package_name_no_match",
			content:   `<PackageReference Include="MyCompany.Services-ios" Version="1.0" />`,
			wantFw:    "",
			wantType:  "",
			wantFound: false,
		},
		{
			// Bare "-android" in a non-TFM context must NOT trigger dotnet-mobile.
			name:      "bare_android_in_condition_no_match",
			content:   `<Condition>'$(Platform)'=='-android'</Condition>`,
			wantFw:    "",
			wantType:  "",
			wantFound: false,
		},
		{
			// Mobile TFM WITH Microsoft.Maui reference → maui.
			name:      "maui_android_with_maui_ref",
			content:   `<TargetFrameworks>net8.0-android</TargetFrameworks><PackageReference Include="Microsoft.Maui.Controls" />`,
			wantFw:    "maui",
			wantType:  "native",
			wantFound: true,
		},
...
func TestBuildNestedMap(t *testing.T) {
	...
	t.Run("leaf key and nested key under same prefix do not panic", func(t *testing.T) {
		// "a" is a leaf (string) but "a.b" tries to descend into "a". The guarded
		// type assertion must recover and create a new nested map rather than panic.
		flat := map[string]string{
			"a":   "leaf-value",
			"a.b": "nested-value",
		}
		// Must not panic; result should at minimum contain the nested key.
		got := buildNestedMap(flat)
		require.NotNil(t, got)
		aVal, ok := got["a"].(map[string]interface{})
		require.True(t, ok, "expected 'a' to be promoted to a nested map")
		assert.Equal(t, "nested-value", aVal["b"])
	})
}
...
// TestGenerateAndWriteQuickstartConfig_SpecialChars verifies that special characters
// in resolved values are properly escaped in XML, TS, and Dart output formats.
func TestGenerateAndWriteQuickstartConfig_SpecialChars(t *testing.T) {
	t.Parallel()

	// Use a client whose secret contains XML/quote special characters to exercise escaping.
	clientID := "cid&<>\"'"
	clientSecret := "secret&<>\"'"
	client := &management.Client{
		ClientID:     &clientID,
	...
	t.Run("xml format escapes special characters", func(t *testing.T) {
		t.Parallel()
		dir := t.TempDir()
		strategy := auth0.FileOutputStrategy{
			Path:   filepath.Join(dir, "Web.config"),
			Format: "xml",
		}
		envValues := map[string]string{
			"auth0:Domain":       auth0.DetectionSub,
		...
		require.NoError(t, err)
		content := string(data)
		// The raw special characters must not appear unescaped in the XML value.
		assert.NotContains(t, content, `value="cid&<>"`)
		// Proper XML entities should be present.
		assert.Contains(t, content, "cid&amp;&lt;&gt;")
	})

	t.Run("ts format escapes single quotes in values", func(t *testing.T) {
		t.Parallel()
		dir := t.TempDir()
		strategy := auth0.FileOutputStrategy{
			Path:   filepath.Join(dir, "environment.ts"),
			Format: "ts",
		}
		// Inject a value that contains a single quote.
		clientIDWithQuote := "cid-with'-quote"
		clientWithQuote := &management.Client{ClientID: &clientIDWithQuote}
		envValues := map[string]string{
			"domain":   auth0.DetectionSub,
			"clientId": auth0.DetectionSub,
		}
		...
		require.NoError(t, err)
		content := string(data)
		// Single quote in the value must be escaped so the TS file remains valid.
		assert.Contains(t, content, `cid-with\'-quote`)
	})

	t.Run("dart format escapes single quotes in values", func(t *testing.T) {
		t.Parallel()
		dir := t.TempDir()
		strategy := auth0.FileOutputStrategy{
			Path:   filepath.Join(dir, "auth_config.dart"),
			Format: "dart",
		}
		clientIDWithQuote := "cid-with'-quote"
		clientWithQuote := &management.Client{ClientID: &clientIDWithQuote}
