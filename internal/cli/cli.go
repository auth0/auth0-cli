...
func (c *cli) setupWithAuthentication(ctx context.Context) error {
	...
	var scopresErr config.ErrTokenMissingRequiredScopes
	if errors.As(err, &scopresErr) {
		c.renderer.Warnf("Required scopes have changed (missing: %s). Please log in to re-authorize the CLI.\n", strings.Join(scopresErr.MissingScopes, ", "))
		tenant, err = RunLoginAsUser(ctx, c, scopresErr.MissingScopes, "")
		if err != nil {
			return err
		}
	}
