...
// GetConfigFolder retrieves the folder where the profiles file is stored It
// searches for the xdg environment path first and will secondarily place it in
// the home directory
func (c *Config) GetConfigFolder(xdgPath string) string {
	...
	return filepath.Join(configPath, "auth0")
}

// Init reads in profiles file and ENV variables if set.
func (c *Config) Init() {
