...
func APIKey(input string) error {
	if len(input) == 0 {
		return errors.New("you have not configured API keys yet. To do so, run `auth0 login` which will configure your API keys from Auth0")
